...
// only two calls this component ever makes, so no client library is needed.
      ...
      {visible.map((r) => (
        <article className="fb-row" key={r.id}>
          <div className="fb-row-head">
            ...
            <time dateTime={r.created_at} suppressHydrationWarning>{when(r.created_at)}</time>
          </div>
...
// `initialRows` arrives server-rendered (SSR) so submissions are in the HTML
...
export default function FeedbackClient({ initialRows, configured }) {
  ...
  return (
    <>
      <Form onSubmitted={refresh} />
      <h2 className="fb-board-title">What others have said</h2>
      <Board rows={rows} error={error && !rows} />
    </>
  );
}
